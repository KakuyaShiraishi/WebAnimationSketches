...
var genericPointColors = ["0xF6318C", "0x36D1C4"]; /*"0x004b85", "0xcc3333"*/
...
function createGenericPoint() {
	momentGenericPoint = new PIXI.Container();
	...
	function onAssetsLoaded() {
		...
		genericPointParticleContainer = new PIXI.Container();
		...
		if (matchVizViewConfig.isWebGL) genericPointCircleMain1.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		momentGenericPoint.addChild(genericPointCircleMain1);
		
		genericPointCircleMain2 = new PIXI.Graphics();
		if (matchVizViewConfig.isWebGL) genericPointCircleMain2.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		momentGenericPoint.addChild(genericPointCircleMain2);
		
		genericPointCircleMain3 = new PIXI.Graphics();
		if (matchVizViewConfig.isWebGL) genericPointCircleMain3.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		momentGenericPoint.addChild(genericPointCircleMain3);
		
		genericPointCircleSmall = new PIXI.Graphics();
		if (matchVizViewConfig.isWebGL) genericPointCircleSmall.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		for (var i = 0; i < 60; i++) {
			...
			if (matchVizViewConfig.isWebGL) genericPointParticle.blendMode = PIXI.BLEND_MODES.MULTIPLY;
			momentGenericPoint.addChild(genericPointParticle);
			genericPointExplosionArray.push(genericPointParticle);
		}

		genericPointTitle = new PIXI.Text("KAKUYA", { font: "120px Knockout47", fill: "#ffffff", align: "center", padding: 15 });
		genericPointTitle.anchor.x = 0.5;
		genericPointTitle.anchor.y = 0.35;
...
function redrawGenericPoint() {
	...
	if (tempColorOrder === 1) {
		genericPointColors = ["0x7ceecc", "0xffebae"]; /*0x004b85", "0xcc3333*/
	} else {
		genericPointColors = ["0xffebae", "0x7ceecc"];
	}
...
function explodeGenericPoint() {
	...
	genericPointAnimationIn.add(TweenMax.fromTo( genericPointTitle, 1,{ x: 0, y: 0, scaleX: 0, scaleY: 0 },{ x: 0, y: 0, scaleX: 1, scaleY: 1, ease: Elastic.easeOut }), .75);
	genericPointAnimationIn.add(TweenMax.fromTo( genericPointTitle, 1,{ alpha: 0 },{ alpha: 1, ease: Expo.easeOut }), .75);
