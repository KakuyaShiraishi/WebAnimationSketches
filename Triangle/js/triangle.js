...
var gameColors = ["0xD89FFF", "0xa9edff"];
...
function createGame() {
	...
	function onAssetsLoaded() {
		...
		gameParticleContainer = new PIXI.Container();
		...
		gameEm_Triangles = new Emitter({
			...
			{ 	type 		:	PolygonParticle,
				...
				speed		: 	[1, 3],
				scale		: 	[.25, 1],
				colors		: 	[gameColors[0], gameColors[1], 0xFFABE5, 0xF6FCAE],
				fade		: 	0.4,
		...
		for (var i = 0; i < 4; i++) {
			...
			if (matchVizViewConfig.isWebGL) gameAngle.blendMode = PIXI.BLEND_MODES.MULTIPLY;
			momentGame.addChild(gameAngle);
			gameAngleArray.push(gameAngle);
		}
		
		gameCircle1 = new PIXI.Graphics();
		if (matchVizViewConfig.isWebGL) gameCircle1.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		momentGame.addChild(gameCircle1);

		gameCircle2 = new PIXI.Graphics();
		if (matchVizViewConfig.isWebGL) gameCircle2.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		momentGame.addChild(gameCircle2);

		gameBarTexture = PIXI.Texture.fromImage("img/common/bar.png");
		gameBar1 = new PIXI.Sprite(gameBarTexture);
		gameBar1.anchor.x = 0.5;
		gameBar1.anchor.y = 0.5;
		gameBar1.tint = gameColors[1];
		if (matchVizViewConfig.isWebGL) gameBar1.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		if (matchVizViewConfig.isWebGL) gameBar2.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		for (var i = 0; i < 60; i++) {
			...
			if (matchVizViewConfig.isWebGL) gameParticle.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		for (var i = 0; i < 7; i++) {
			...
			if (matchVizViewConfig.isWebGL) gameParticleTriangle.blendMode = PIXI.BLEND_MODES.MULTIPLY;
			momentGame.addChild(gameParticleTriangle);
			gameExplosionArray3.push(gameParticleTriangle);
		}
		
		gameTitle = new PIXI.Text("KAKUYA", { font: "125px Knockout47", fill: "#ffffff", align: "center", padding: 15});
		gameTitle.anchor.x = 0.5;
		gameTitle.anchor.y = 0.55;
...
function redrawGame() {
	...
	if (tempColorOrder === 1) {
		gameColors = ["0xD89FFF", "0xa9edff"];
	} else {
		gameColors = ["0xa9edff", "0xD89FFF"];
	}
...
function explodeGame() {
	...
	gameAnimationIn.add(TweenMax.fromTo( gameTitle, 1,{ x: 0, y: 0, scaleX: 0, scaleY: 0 },{ x: 0, y: -25, scaleX: 1, scaleY: 1, ease: Elastic.easeOut }), 3);
	gameAnimationIn.add(TweenMax.fromTo( gameTitle, 1,{ alpha: 0 },{ alpha: 1, ease: Expo.easeOut }), 3);
