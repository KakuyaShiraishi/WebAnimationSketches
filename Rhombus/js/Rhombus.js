...
var setPointColors = ["0xF9F296", "0x8c60dc"]; /*0xf39909", "0xa51981*/
...
function createSetPoint() {
	momentSetPoint = new PIXI.Container();
	momentGroup.addChild(momentSetPoint);
	
	var loader = new PIXI.loaders.Loader();
	loader.add("img/background.png");
	loader.once('complete',onAssetsLoaded);
	loader.load();
	
	function onAssetsLoaded() {
		
		setPointParticleContainer = new PIXI.Container();
		...
		if (matchVizViewConfig.isWebGL) setPointCircle1.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		momentSetPoint.addChild(setPointCircle1);
		
		setPointCircle2 = new PIXI.Graphics();
		if (matchVizViewConfig.isWebGL) setPointCircle2.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		momentSetPoint.addChild(setPointCircle2);
		
		setPointSquare1 = new PIXI.Graphics();
		if (matchVizViewConfig.isWebGL) setPointSquare1.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		if (matchVizViewConfig.isWebGL) setPointSquare2.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		if (matchVizViewConfig.isWebGL) setPointLaser1.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		if (matchVizViewConfig.isWebGL) setPointLaser2.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		for (var i = 0; i < 60; i++) {
			...
			if (matchVizViewConfig.isWebGL) setPointParticle.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		for (var i = 0; i < 7; i++) {
			...
			if (matchVizViewConfig.isWebGL) setPointParticleSquare.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		for (var i = 0; i < 5; i++) {
			...
			if (matchVizViewConfig.isWebGL) setPointParticleSquareMini.blendMode = PIXI.BLEND_MODES.MULTIPLY;
		...
		for (var i = 0; i < 12; i++) {
			...
			if (matchVizViewConfig.isWebGL) setPointRectangles.blendMode = PIXI.BLEND_MODES.MULTIPLY;
			momentSetPoint.addChild(setPointRectangles);
			setPointRectangleArray.push(setPointRectangles);
		}
		
		setPointTitle = new PIXI.Text("KAKUYA", { font: "130px Knockout47", fill: "#ffffff", align: "center", padding: 15 });
		setPointTitle.anchor.x = 0.5;
		setPointTitle.anchor.y = 0.35;
...
function redrawSetPoint() {
	...
	if (tempColorOrder === 1) {
		setPointColors = ["0xFCD78E", "0xF3F169"]; /*0xf39909", "0xa51981*/
	} else {
		setPointColors = ["0xF3F169", "0xFCD78E"];
	}
...
function explodeSetPoint() {
	...
	setPointAnimationIn.add(TweenMax.fromTo( setPointTitle, 1,{ x: 0, y: 0, scaleX: 0, scaleY: 0 },{ x: 0, y: 0, scaleX: 1, scaleY: 1, ease: Elastic.easeOut }), 3);
	setPointAnimationIn.add(TweenMax.fromTo( setPointTitle, 1,{ alpha: 0 },{ alpha: 1, ease: Expo.easeOut }), 3);
